 ...
 * Priority: env var → current-run-id file → auto-generate.
 * This ensures all agents in a /ceo or /ship run share one trace directory.
 */
function resolveTraceRunId(projectRoot: string): string {
	// 1. Already set (parent explicitly passed it)
	if (process.env.TRACE_RUN_ID) return process.env.TRACE_RUN_ID;

	// 2. Read from file written by trace-recorder
	const runIdFile = path.join(projectRoot, ".pi", "traces", "current-run-id");
	try {
		if (fs.existsSync(runIdFile)) {
			const id = fs.readFileSync(runIdFile, "utf-8").trim();
			if (id) return id;
		}
	} catch {
		// Fall through
...
async function runSingleAgent(
	...
	// recognized by --tools and would be silently dropped with a warning.
	// Extensions auto-discover from .pi/extensions/ in the CWD, so 'subagent'
	// becomes available automatically even if not passed to --tools.
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			const proc = spawn("pi", args, {
				...
				env: {
					...process.env,
					// Propagate trace run ID so subagents land in the same trace directory.
